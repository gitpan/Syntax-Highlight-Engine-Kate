...
# This file was generated from the 'katexml/d.xml' file of the syntax highlight
...
sub parseLinkage {
	my ($self, $text) = @_;
	# type => keyword
	if ($self->testKeyword($text, 'types', 0, 0, undef, 0, '#pop', 'Type')) {
		return 1
	}
	# type => DetectChar
	...
	# type => keyword
	if ($self->testKeyword($text, 'ltypes', 0, 0, undef, 0, '#stay', 'Linkage Type')) {
		return 1
	}
	# type => DetectChar
...
sub parsePragmas {
	...
	# type => keyword
	if ($self->testKeyword($text, 'ptypes', 0, 0, undef, 0, '#stay', 'Version Type')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'vtypes', 0, 0, undef, 0, '#stay', 'Version Type')) {
		return 1
	}
	# type => RegExpr
...
sub parsenormal {
	...
	# type => keyword
	if ($self->testKeyword($text, 'modules', 0, 0, undef, 0, 'ModuleName', 'Module')) {
		return 1
	}
	# type => keyword
	...
	# type => keyword
	if ($self->testKeyword($text, 'phobos', 0, 0, undef, 0, '#stay', 'Phobos Library')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'linkage', 0, 0, undef, 0, 'Linkage', 'Linkage')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'debug', 0, 0, undef, 0, '#stay', 'Debug')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'assert', 0, 0, undef, 0, '#stay', 'Assert')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'pragma', 0, 0, undef, 0, 'Pragmas', 'Pragma')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'version', 0, 0, undef, 0, 'Version', 'Version')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'unittest', 0, 0, undef, 0, '#stay', 'Unit Test')) {
		return 1
	}
	# type => Detect2Chars
