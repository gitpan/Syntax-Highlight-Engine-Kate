...
# This file was generated from the 'katexml/povray.xml' file of the syntax highlight
...
sub parseCommentar1 {
	my ($self, $text) = @_;
	# type => keyword
	if ($self->testKeyword($text, 'attention', 0, 0, undef, 0, '#stay', 'Decimal')) {
		return 1
	}
	return 0;
};
...
sub parseNormal {
	...
	# type => keyword
	if ($self->testKeyword($text, 'objects', 0, 0, undef, 0, '#stay', 'Objects')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'texturing', 0, 0, undef, 0, '#stay', 'Texturing')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'transforms', 0, 0, undef, 0, '#stay', 'Transforms')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'filetypes', 0, 0, undef, 0, '#stay', 'Filetype')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'identifiers', 0, 0, undef, 0, '#stay', 'Keyword')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'constants', 0, 0, undef, 0, '#stay', 'Constant')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'functions', 0, 0, undef, 0, '#stay', 'Functions')) {
		return 1
	}
	# type => keyword
	...
	# type => Int
	if ($self->testInt($text, 0, 0, undef, 0, '#stay', 'Float')) {
		return 1
	}
	# type => HlCChar
