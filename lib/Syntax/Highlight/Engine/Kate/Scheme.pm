...
# This file was generated from the 'katexml/scheme.xml' file of the syntax highlight
...
sub parseDefault {
	...
	# type => keyword
	if ($self->testKeyword($text, 'defines', 0, 0, undef, 0, 'function_decl', 'Definition')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'characters', 0, 0, undef, 0, '#stay', 'Char')) {
		return 1
	}
	# type => RegExpr
...
sub parseSpecialNumber {
	my ($self, $text) = @_;
	# type => Float
	if ($self->testFloat($text, 0, 0, undef, 0, '#pop', 'Float')) {
		return 1
	}
	# type => Int
	if ($self->testInt($text, 0, 0, undef, 0, '#pop', 'Decimal')) {
		return 1
	}
	# type => HlCOct
