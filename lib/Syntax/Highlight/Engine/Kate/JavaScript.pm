...
# This file was generated from the 'katexml/javascript.xml' file of the syntax highlight
...
sub parseNormal {
	...
	# type => keyword
	if ($self->testKeyword($text, 'math', 0, 0, undef, 0, '#stay', 'Math')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'events', 0, 0, undef, 0, '#stay', 'Events')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'methods', 0, 0, undef, 0, '#stay', 'Data Type')) {
		return 1
	}
	# type => DetectIdentifier
	if ($self->testDetectIdentifier($text, 0, undef, 0, '#stay', undef)) {
