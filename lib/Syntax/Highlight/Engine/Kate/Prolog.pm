...
# This file was generated from the 'katexml/prolog.xml' file of the syntax highlight
...
sub parsenormal {
	my ($self, $text) = @_;
	# type => keyword
	if ($self->testKeyword($text, 'keywordl', 0, 0, undef, 0, '#stay', 'Keyword')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'keywordu', 0, 0, undef, 0, '#stay', 'Keyword')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'arith', 0, 0, undef, 0, '#stay', 'Arithmetic')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'compiler', 0, 0, undef, 0, '#stay', 'Keyword')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'special', 0, 0, undef, 0, '#stay', 'Keyword')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'basetype', 0, 0, undef, 0, '#stay', 'Data Type')) {
		return 1
	}
	# type => RegExpr
