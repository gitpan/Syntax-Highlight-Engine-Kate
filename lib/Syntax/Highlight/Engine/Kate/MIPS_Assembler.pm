...
# This file was generated from the 'katexml/mips.xml' file of the syntax highlight
# engine of the kate text editor (http://kate.kde.org

#kate xml version 1.03
#kate version 2.4
#kate author Dominik Haumann (dhdev@gmx.de)
#generated: Sun May 28 21:18:37 2006, localtime
...
sub parsenormal {
	my ($self, $text) = @_;
	# type => keyword
	if ($self->testKeyword($text, 'hardware', 0, 0, undef, 0, '#stay', 'Hardware Instruction')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'pseudo', 0, 0, undef, 0, '#stay', 'Pseudo Instruction')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'register1', 0, 0, undef, 0, '#stay', 'Other Register')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'register2', 0, 0, undef, 0, '#stay', 'Main Register')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'fp', 0, 0, undef, 0, '#stay', 'Floating Point Register')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'section', 0, 0, undef, 0, '#stay', 'Type')) {
		return 1
	}
	# type => keyword
	if ($self->testKeyword($text, 'type', 0, 0, undef, 0, '#stay', 'Section')) {
		return 1
	}
	# type => RegExpr
